...
class Service {
    ...
    async create(data, params) {
        ...
        if (accessToken !== undefined) {
            ...
            if (userRole.length > 0) {
                ...
                const patientDefaultPaymentPlan = patientDetails.paymentPlan.find(x => x.isDefault);
                ...
                try {
                    // Use the bill creator to generate bills for the items that has been billed.
                    let createBill = await billCreatorService.create(billItems, {
                        query: {
    ...
    //                 if (getBill._id) {
    //                     const containsIsBilled = unBilledArray.filter(x => x.isBilled);
    //                     console.log('containsIsBilled => ', containsIsBilled);

    //                     let totalCost = 0;
    //                     let totalQuantity = 0;
    //                     if (containsIsBilled.length > 0) {
    //                         const patientDetails = await patientService.get(getBill.patientId);
    //                         const patientDefaultPaymentPlan = patientDetails.paymentPlan.find(x => x.isDefault);
    //                         console.log('patientDefaultPaymentPlan => ', patientDefaultPaymentPlan);
    ...
    //                         getBill.subTotal += totalCost;
    //                         getBill.grandTotal += totalCost;
    //                         try {
    //                             // Update the Billing service
    //                             const patchBill = await billingService.patch(getBill._id, getBill, {});
    ...
    //                                 } catch (e) {
    //                                     console.log(e);
    //                                     return jsend.error('There was a problem updating prescription service.');
    //                                 }
    //                             }
    //                         } catch (e) {
    //                             console.log(e);
    //                             return jsend.error('There was a problem updating billing service.');
    //                         }
    //                     } else {
