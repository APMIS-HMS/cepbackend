  ...
  joins: {
    productObject: () => async (data, context) => {
      ...
      productReorder: () => async (data, context) => {
        ...
        storeObject: () => async (data, context) => {
          try {
            const getStore = await context.app.service('stores').get(data.storeId);
            const facilityPrice = await context.app.service('facility-prices').find({
              query: {
                facilityId: data.facilityId,
                categoryId: data.categoryId,
                facilityServiceId: data.facilityServiceId,
                serviceId: data.serviceId,
                $select: ['modifiers','price']
              }
            });
            data.store = getStore.name;
            data.price = facilityPrice.data[0];
