...
class Service {
  ...
  async create(data, params) {
    try {
      ...
      const pay = await userService.find({
        query: {
      ...
      if (pay.data.length > 0) {
        ...
        let fms = await facilityModuleService.find({
          query: {
            canDisable: false
          }
        });
        ...
        const allFacilityFeatures = await featuresService.find({
          query: {
            name: 'Facility'
          }
        });
        ...
        var l = facilityFeatureData.length > 0 ? facilityFeatureData[0].actions.length : 0;
        while (l--) {
        ...
        const allAccessFeatures = await featuresService.find({
          query: {
            name: 'Access Control'
          }
        });
        ...
        var k = accessFeatureData.length > 0 ? accessFeatureData[0].actions.length : 0
        while (k--) {
        ...
        const payload = await userService.patch(user._id, {
